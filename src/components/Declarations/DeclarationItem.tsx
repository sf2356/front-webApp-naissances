...
function DeclarationItem({declaration, index}: {declaration: Declaration, index: number}) {
  return (
    <div>
        <article  className={`grid grid-cols-12 align-center p-4 border border-gray-300 ${index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}  `}>
            ...
            <StatusBar status={getStatusColor(declaration.status)} label={getStatusTextFrench(declaration.status)} />
              <ActionButton
                classes="p-2 col-span-2 flex flex-col"
                label="Action"
                action={() => null}
              />
          </article>
